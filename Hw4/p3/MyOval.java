...
public class MyOval extends MyFillableShape{
	private int mWidth;
	private int mHeight;
	
	public MyOval(Point position, Color color, int width, int height, boolean fill) {
	...
	public void draw(Graphics g) {
		g.setColor(getColor());
		if (isFill()) {
			g.fillOval(getX(), getY(), mWidth, mHeight);
		} else {
			g.drawOval(getX(), getY(), mWidth, mHeight);
		}
	}
